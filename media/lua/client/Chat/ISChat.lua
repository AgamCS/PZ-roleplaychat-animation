...
-- Emotes are in a different table than handsigns as not all the handsigns make sense in a conversation
local emoteSigns = {
    "undecided",
    "thankyou",
    "wavehi",
...
ISChat.onTextChange = function()
    ...
    local player = getPlayer();
    --print("Server time " .. getTimestamp())
    --print("Next emote time " .. nextEmoteTime)
    if ISChat.instance.chatText.lastChatCommand ~= nil then
        if getTimestamp() > nextEmoteTime then
            local index = ZombRand(1, 6);
            --print(index)
            --print(emoteSigns[index])
            player:playEmote(emoteSigns[index]);
            nextEmoteTime = getTimestamp() + 3;   
        end
        --print("emote done")
